...
import {
  ...
  validarDocumento,
  validarEmail,
} from '../domain/inquilino';
import { criarInquilino } from '../infrastructure/inquilinos-api';

interface AddInquilinoPageProps {
...
export function AddInquilinoPage({ company, onCancel, onCriado }: AddInquilinoPageProps) {
  ...
  const handleCriar = async () => {
    ...
    if (!documento.trim()) {
      setError("Informe o CPF ou CNPJ do inquilino");
      return;
    }
    ...
    setLoading(true);
    try {
      await criarInquilino(empresaHandle, {
        nome: nome.trim(),
        documento: normalizarDocumento(documento),
        telefone: telefone.trim() || null,
        email: email.trim() || null,
      });
      setSuccess(`Inquilino ${nome.trim()} cadastrado com sucesso.`);
      setNome("");
      setDocumento("");
      setTelefone("");
      setEmail("");
      ...
      setError((err instanceof Error ? err.message : "") || "Erro ao cadastrar inquilino");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="mb-6">
        <Breadcrumb className="mb-2">
          <BreadcrumbList className="text-[13px]">
            <BreadcrumbItem>Inquilinos</BreadcrumbItem>
            <BreadcrumbSeparator className="[&>svg]:size-3 [&>svg]:text-faint" />
            <BreadcrumbItem>
              <BreadcrumbPage className="font-medium">Novo inquilino</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
        ...
        <p className="text-sm text-muted-foreground">
          Cadastre o inquilino de um imóvel para vincular futuras vistorias.
        </p>
      </div>

      <Card className="max-w-2xl gap-5 p-5">
        <div className="grid gap-1.5">
          <Label htmlFor="inquilino-nome" className={LABEL_CLASS}>Nome</Label>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="grid gap-1.5">
            <Label htmlFor="inquilino-telefone" className={LABEL_CLASS}>
          ...
          <div className="grid gap-1.5">
            <Label htmlFor="inquilino-email" className={LABEL_CLASS}>
